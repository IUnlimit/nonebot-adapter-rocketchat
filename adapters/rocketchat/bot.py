...
class Bot(BaseBot):
    ...
    async def handle_event(self, event: Event):
        # exclude self event
        if getattr(event, 'u') and event.u.username == self.self_id:
            return
    ...
    async def send(
        ...
        **kwargs,
        ...
        if isinstance(event, RoomMessageEvent) is not True:
            raise Exception("Unsupport")
        event: RoomMessageEvent = event
        # 将消息处理为平台所需的格式后，调用发送消息接口进行发送，例如：
        # data = message_to_platform_data(message)
        rest_api.send_message(str(message), event.rid)
